...
metadata {
	...
	definition(name: "TH1123ZB-TH1124ZB Sinope Thermostat", namespace: "Sinope Technologies", author: "Sinope Technologie") {
...
void initialize() {
	...
	//	int delay=7 // run every 7 minutes     
	//	schedule("0 0/${delay} * * * ?", refresh_misc)
...
def refresh() {
	...
	cmds += zigbee.configureReporting(0x0201, 0x0008, 0x0020, 11, 61, 10) 	//heating demand
	cmds += zigbee.configureReporting(0x0201, 0x0012, 0x0029, 0, 67, 40) 	//occupied heating setpoint reported only on change
...
void refresh_misc() {
	...
	if (weather) {
		...
		long secFrom2000 = (((now().toBigInteger() + mytimezone.rawOffset) / 1000) - (10956 * 24 * 3600)).toLong() //number of second from 2000-01-01 00:00:00h
